...
int main() {
  ...
  if (signal == 0) {
    ...
    .CHECK_OUTPUT(f1, "7.05[km]")
    .CHECK_OUTPUT(f1+f1, "14.1[km]")
    ...
    .CHECK_OUTPUT(+e2, "-5[km]")
    .CHECK_OK(e2++)
    .CHECK_OUTPUT(e2, "-4[km]")
    .CHECK_OK(f2--)
    .CHECK_OUTPUT(f2, "-8[km]")
